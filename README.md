# 印记相机 · MarkCam 1.1.0 测试版

原生 Objective-C iPhone 相机，重点是“先设计水印，之后每次拍摄自动应用”。

**交付状态：1.1.0（build 3）新增原生 Live Photo、EV 设置和主屏缩放滑杆，已交叉编译、链接、打包及静态回归；新增功能仍待真机复测。**
用户已确认 1.0.1 拍照恢复正常。本版本保留此前的代理修复；Live Photo 的相册长按播放、音频、水印与方向必须在实际设备上验收。
...
## 下载文件
在本仓库 **Releases → v1.1.0** 下载 IPA、源码包及校验报告。二进制不放进 Git 历史，避免仓库膨胀。
- `MarkCam-1.1.0-resign-required.ipa`：需要重签的测试包。
...
## 首版限制 / 未验证事项
- 新增 Live Photo、EV 持久化和缩放 UI 尚待真机复测；旧版 1.0.1 拍照正常已获用户确认，但不代表所有录像/方向/性能路径均已验收。
...
- 170项 IPA 结构、二进制方法表、源代码断言，以及36项主机侧倍率/布局数学模型和静态契约检查通过。这些不是相机运行时功能测试。
- 专项编译回归通过：正确回调可编译；故意把三个照片/实况回调恢复为错误前缀后，编译必须报错。
- 二进制方法表确认普通照片、Live Photo 回调与处理器确实链接进 App，不仅依赖源码字符串匹配。
- 未运行 iOS 模拟器，新版 Live Photo 相册播放、音频/方向/水印/性能仍需真机复测。
