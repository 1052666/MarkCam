# 印记相机 · MarkCam 1.1.1 测试版

原生 Objective-C iPhone 相机，重点是“先设计水印，之后每次拍摄自动应用”。

**交付状态：1.1.1（build 4）合并取景性能优化、大取景布局与真实后置超广角，已交叉编译、链接及回归检查；新增功能仍待真机复测。**
用户已确认1.0.1拍照恢复正常，随后反馈1.1.0取景卡顿、画面偏小。本版修复已确定的预览实现问题，但不宣称达到iOS原相机帧率或实测完成。
...
## 下载文件
在本仓库 **Releases → v1.1.1** 下载 IPA、源码包及校验报告。二进制不放进 Git 历史，避免仓库膨胀。
- `MarkCam-1.1.1-resign-required.ipa`：需要重签的测试包。
...
- 193项 IPA结构/二进制/源码断言、506项主机原生C布局/倍率与性能契约检查、20项实况/曝光契约检查通过。检查数量不代表真机功能测试数量。
