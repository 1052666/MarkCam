...
- (BOOL)allowsCaptureLive:(BOOL)live video:(BOOL)video {
 ...
 return MCCanStartCapture((unsigned)self.pendingCount,live,video,self.processing,self.heavyProcessing,pressure,os_proc_available_memory());
}
...
- (void)tick {
 // Capture availability can change without any job changing (pressure timeout,
 // thermal recovery or available memory). Refresh controls even on early exits.
 [self notify];
 if(self.processing)return;if(self.scanning)return;
...
- (void)save:(NSMutableDictionary *)job meta:(NSURL *)meta {
 ...
 if(st==PHAuthorizationStatusNotDetermined){
  // The first shutter press must capture and persist its photo before asking
  // about saving. Keep ownership of this job while the permission sheet is up.
  [PHPhotoLibrary requestAuthorizationForAccessLevel:PHAccessLevelAddOnly handler:^(PHAuthorizationStatus status){
   dispatch_async(dispatch_get_main_queue(),^{[self save:job meta:meta];});
  }];return;
 }
