...
camera=(ROOT/'Sources/CameraViewController.m').read_text();gpu=(ROOT/'Sources/MCPreviewView.m').read_text()
...
(ROOT/'dist/preview-regression-report.json').write_text(json.dumps(report,indent=2,ensure_ascii=False)+'\n')
(ROOT/'tests/layout-fixtures.json').write_text(json.dumps(layouts,indent=2)+'\n')
